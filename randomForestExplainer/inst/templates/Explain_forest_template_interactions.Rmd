...
The plots below show predictions of the random forest depending on values of components of an interaction (the values of remaining predictors are sampled from their empirical distribution) for up to three most frequent interactions that consist of two numerical variables.
...
interactions_frame <- interactions_frame[interactions_frame$variable != interactions_frame$root_variable, ]
if(nrow(interactions_frame) > 0){
  ...
  for(i in 1:nrow(selected)){
    print(plot_predict_interaction(forest, data, paste0(selected$root_variable[i]), paste0(selected$variable[i])))
  }
} else {
