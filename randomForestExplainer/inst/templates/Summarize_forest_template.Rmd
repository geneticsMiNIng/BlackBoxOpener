...
```{r}
formatRound(datatable(importance_frame), c(2,4,5), digits = 4)
```
...
The second multi-way importance plot shows two importance measures that derive from the role a variable plays in prediction: `r print(noquote(measures_print))` with the additional information on the number of trees in which a variable was used for splitting.

```{r}
plot_multi_way_importance(importance_frame, x_measure = measure1, y_measure = measure2, size_measure = "no_of_trees")
```
...
```{r conditional_print, child = 'Summarize_forest_template_interactions.Rmd', eval = interactions}
```
